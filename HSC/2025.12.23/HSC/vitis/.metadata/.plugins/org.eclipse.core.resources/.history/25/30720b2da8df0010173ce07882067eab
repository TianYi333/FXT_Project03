...
volatile uint32_t frame_bytes = 16000; 		// 帧字节数
...
int AXI_DMA_APP(void)
{
    ...
    {
//		vTaskDelay(1);
		if(xSemaphoreTake(xUpdate_flagSemaphore,1))
		{
...
//			Xil_DCacheFlushRange((UINTPTR) rx_buffer_ptr, MAX_PKT_LEN);
			...
			// 处理接收到的数据
			ProcessReceivedData(frame_bytes);
...
{
    u8 Tx_Axi_Stream_buf[8]={0};
    Tx_Axi_Stream_buf[0]=(ActualBytes>>24)&0xff;//数组0位为接收数据数
    Tx_Axi_Stream_buf[1]=(ActualBytes>>16)&0xff;//数组0位为接收数据数
    Tx_Axi_Stream_buf[2]=(ActualBytes>>8)&0xff;//数组0位为接收数据数
    Tx_Axi_Stream_buf[3]=ActualBytes&0xff;//数组0位为接收数据数
//    xQueueSend(Lwip_Send_Queue,Tx_Axi_Stream_buf,0);
	memset(Tx_Axi_Stream_buf, 0, 4);
	PL_read_bytes = PL_read_bytes + ActualBytes;
	xil_printf("PL_read_bytes= %d \r\n", PL_read_bytes);
	if(PL_read_bytes >= bytes && PL_read_bytes < bytes*2)
	{
		if((frame_count%3) == 0)
		{
			Tx_Axi_Stream_buf[0]=(PL_read_bytes>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[1]=(PL_read_bytes>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[2]=(PL_read_bytes>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[3]=PL_read_bytes&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[4]=(frame_count>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[5]=(frame_count>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[6]=(frame_count>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[7]=frame_count&0xff;//数组0位为接收数据数
			xQueueSend(Lwip_Send_Queue,Tx_Axi_Stream_buf,0);
			frame_count++;
			PL_read_bytes= MAX_PKT_LEN;
		}
	}
	else if(PL_read_bytes >= (bytes+MAX_PKT_LEN) && PL_read_bytes < (bytes*2+MAX_PKT_LEN))
	{
		if((frame_count%3) == 1)
		{
			Tx_Axi_Stream_buf[0]=((PL_read_bytes-MAX_PKT_LEN)>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[1]=((PL_read_bytes-MAX_PKT_LEN)>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[2]=((PL_read_bytes-MAX_PKT_LEN)>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[3]=(PL_read_bytes-MAX_PKT_LEN)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[4]=(frame_count>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[5]=(frame_count>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[6]=(frame_count>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[7]=frame_count&0xff;//数组0位为接收数据数
	...
	else if(PL_read_bytes >= (bytes+MAX_PKT_LEN*2) && PL_read_bytes < (bytes*2+MAX_PKT_LEN*2))
	{
		if((frame_count%3) == 2)
		{
			Tx_Axi_Stream_buf[0]=((PL_read_bytes-MAX_PKT_LEN*2)>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[1]=((PL_read_bytes-MAX_PKT_LEN*2)>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[2]=((PL_read_bytes-MAX_PKT_LEN*2)>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[3]=(PL_read_bytes-MAX_PKT_LEN*2)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[4]=(frame_count>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[5]=(frame_count>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[6]=(frame_count>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[7]=frame_count&0xff;//数组0位为接收数据数
