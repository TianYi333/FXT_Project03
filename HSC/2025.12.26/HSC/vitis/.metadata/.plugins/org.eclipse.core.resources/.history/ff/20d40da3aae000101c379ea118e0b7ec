...
{
    u8 Tx_Axi_Stream_buf[9]={0};
//    Tx_Axi_Stream_buf[0]=0xFF;
//    Tx_Axi_Stream_buf[1]=(ActualBytes>>24)&0xff;//数组0位为接收数据数
//    Tx_Axi_Stream_buf[2]=(ActualBytes>>16)&0xff;//数组0位为接收数据数
//    Tx_Axi_Stream_buf[3]=(ActualBytes>>8)&0xff;//数组0位为接收数据数
//    Tx_Axi_Stream_buf[4]=ActualBytes&0xff;//数组0位为接收数据数
//    xQueueSend(Lwip_Send_Queue,Tx_Axi_Stream_buf,0);
//	memset(Tx_Axi_Stream_buf, 0, 5);
...
//	xil_printf("PL_read_bytes= %d \r\n", PL_read_bytes);
	if(PL_read_bytes >= bytes && PL_read_bytes < bytes*2)
	{
		...
		{
			Tx_Axi_Stream_buf[0]=0xFF;
			Tx_Axi_Stream_buf[1]=(PL_read_bytes>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[2]=(PL_read_bytes>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[3]=(PL_read_bytes>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[4]=PL_read_bytes&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[5]=(frame_count>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[6]=(frame_count>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[7]=(frame_count>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[8]=frame_count&0xff;//数组0位为接收数据数
			xQueueSendFromISR(Lwip_Send_Queue,Tx_Axi_Stream_buf,0);
			frame_count++;
	...
	else if(PL_read_bytes >= (bytes+MAX_PKT_LEN) && PL_read_bytes < (bytes*2+MAX_PKT_LEN))
	{
		...
		{
			Tx_Axi_Stream_buf[0]=0xFF;
			Tx_Axi_Stream_buf[1]=((PL_read_bytes-MAX_PKT_LEN)>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[2]=((PL_read_bytes-MAX_PKT_LEN)>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[3]=((PL_read_bytes-MAX_PKT_LEN)>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[4]=(PL_read_bytes-MAX_PKT_LEN)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[5]=(frame_count>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[6]=(frame_count>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[7]=(frame_count>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[8]=frame_count&0xff;//数组0位为接收数据数
	...
	else if(PL_read_bytes >= (bytes+MAX_PKT_LEN*2) && PL_read_bytes < (bytes*2+MAX_PKT_LEN*2))
	{
		...
		{
			Tx_Axi_Stream_buf[0]=0xFF;
			Tx_Axi_Stream_buf[1]=((PL_read_bytes-MAX_PKT_LEN*2)>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[2]=((PL_read_bytes-MAX_PKT_LEN*2)>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[3]=((PL_read_bytes-MAX_PKT_LEN*2)>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[4]=(PL_read_bytes-MAX_PKT_LEN*2)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[5]=(frame_count>>24)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[6]=(frame_count>>16)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[7]=(frame_count>>8)&0xff;//数组0位为接收数据数
			Tx_Axi_Stream_buf[8]=frame_count&0xff;//数组0位为接收数据数
