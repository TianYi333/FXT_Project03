...
//    xQueueSend(Lwip_Send_Queue,Tx_Axi_Stream_buf,0);
	...
	if(PL_read_bytes >= MAX_PKT_LEN && PL_read_bytes < MAX_PKT_LEN*2)
	{
		frame_count++;
	...
	else if(PL_read_bytes >= MAX_PKT_LEN*2 &&PL_read_bytes < MAX_PKT_LEN*3)
	{
		frame_count++;
	    Tx_Axi_Stream_buf[0]=(PL_read_bytes>>24)&0xff;//数组0位为接收数据数
	    Tx_Axi_Stream_buf[1]=(PL_read_bytes>>16)&0xff;//数组0位为接收数据数
	...
	else if(PL_read_bytes>=MAX_PKT_LEN*3)
	{
		frame_count++;
	    Tx_Axi_Stream_buf[0]=(PL_read_bytes>>24)&0xff;//数组0位为接收数据数
	    Tx_Axi_Stream_buf[1]=(PL_read_bytes>>16)&0xff;//数组0位为接收数据数
